...
export default function ChatNav({ onOpenCreateChat }) {
    ...
    return (
        <section id="chatComponent" className="text-center bg-white/30 h-3/4 w-1/5 self-center rounded-xl drop-shadow-2xl border border-zinc-200 p-4 overflow-y-auto">
...
function ChatList({ allChats }) {
    return (
        <ul id="chatlist" className="flex flex-col gap-y-0 overflow-y-scroll h-50/100 no-scrollbar tracking-wide font-semibold">
            {allChats.map((chat) => (
...
function ChatItem({ name, id }) {
    ...
    const liClassName = match
        ? "border-none border-c-light-blue bg-blue-200/70 opacity-80 rounded-xl text-gray-100 font-bold font-ubuntu text-white text-lg md:text-2xl"
        : "hover:border-2 hover:border-sage hover:rounded-xl opacity-80 text-zinc-900 mt-2 mx-5 text-gray-100 font-bold font-ubuntu text-white text-xs md:text-2xl";
    return (
    ...
    return (
        <button
           // className="bg-blue-400 shadow-md rounded-xl w-3/5 h-10 mb-3 text-white hover:bg-blue-200  "
            ...
            className="bg-[rgb(18, 18, 18)] shadow-md rounded-xl w-3/5 h-10 mb-3 text-white hover:bg-blue-200 text-xs md:text-lg  "
            onClick={onClick}
